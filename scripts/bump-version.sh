...
if [ -n "$1" ]; then
  ...
  # Make sure all packages csv-stream-lite* depend on latest versions of other csv-stream-lite* packages

  echo "Updating all csv-stream-lite* dependencies to version $NEW_VERSION"

  find . -name 'package.json' -not -path '*/node_modules/*' | while read -r pkg_file; do
    # Only update if there are csv-stream-lite* dependencies
    if grep -q '"csv-stream-lite' "$pkg_file"; then
      tmp_file=$(mktemp)
      node -e "
        const fs = require('fs');
        ...
        ['dependencies', 'devDependencies'].forEach(depType => {
          if (pkg[depType]) {
            Object.keys(pkg[depType]).forEach(dep => {
              if (dep.startsWith('csv-stream-lite')) {
